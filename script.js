...
const customerDetailMainPhotoImage = document.querySelector("#customer-detail-main-photo-image");
...
const customerDetailPhotoInput =document.querySelector("#customer-detail-photo-input");
...
const customerDetailEditFeatureList =document.querySelector("#customer-detail-edit-feature-list");
const customerDetailEditFeatureInput =document.querySelector("#customer-detail-edit-feature-input");
const customerDetailEditFeatureAddButton =document.querySelector(".customer-detail-edit-feature-add-button");
...
// 常連顧客検索で現在表示されている顧客だけ
// 代表写真をGASから遅れて読み込む
const loadVisibleRegularCustomerPhotoPreviews =
  () => {
    if (
    ...
    const photoUrls =
      visiblePhotoItems.map(
        (resultItem) => {
          return resultItem.dataset
            .photoUrl;
        }
      );
            ...
            (resultItem) => {
              ...
              const avatar =
                resultItem.querySelector(
                  ".regular-customer-result-avatar"
                );
              ...
              image.className =
                "regular-customer-result-avatar-image";
...
const updateRegularCustomerSearchResults =
  () => {
    ...
    // 検索結果に表示された顧客だけ代表写真を読み込む
    loadVisibleRegularCustomerPhotoPreviews();
  };
...
const showSelectedRegularCustomer = (
  ...
  // まず頭文字を表示しておき、写真がある場合だけ後から置き換える
  ...
  if (
    ...
    image.className =
      "regular-selected-customer-avatar-image";
    ...
    regularSelectedAvatar.replaceChildren(
      image
    );
  } else if (
    selectedPhotoUrl &&
    ...
    google.script.run
      .withSuccessHandler(
        (photoPreviews) => {
          ...
          // 途中で別のお客様へ変更されていたら反映しない
          if (
            !previewUrl ||
            regularSelectedCustomerIdInput
              .value !==
...
// 現在画面に表示されている顧客だけ
// 代表写真をGASから遅れて読み込む
const loadVisibleCustomerPhotoPreviews =
  () => {
    if (
      !window.google?.script?.run
    ) {
    ...
    const photoUrls =
      visiblePhotoCards.map(
        (resultCard) => {
          return resultCard.dataset
            .photoUrl;
        }
      );
        ...
        (photoPreviews) => {
          ...
          visiblePhotoCards.forEach(
            (resultCard) => {
              ...
              image.className =
                "customer-search-result-avatar-image";
...
const updateCustomerSearchResults = () => {
  ...
  // 現在表示された顧客だけ代表写真を後から読み込む
  loadVisibleCustomerPhotoPreviews();
};
...
const createHomeCustomerCardHtml = (
  ...
  return `
    <button
      ...
      data-home-photo-url="${escapeCustomerSearchHtml(
        photoUrl
      )}"
...
// ホームに現在表示している顧客だけ代表写真を読み込む
const loadHomeCustomerPhotoPreviews =
  () => {
    if (
      !window.google?.script?.run
    ) {
...
const renderHomeRecentCustomers =
  () => {
      ...
      [...customerCards]
        .sort(
          (
            ...
            const registrationDateDifference =
            ...
            // 登録日が違う場合は新しい日付を先にする
            if (
              registrationDateDifference !==
              0
            ) {
              return registrationDateDifference;
            }
    ...
    // ホームに表示された顧客だけ代表写真を読み込む
    loadHomeCustomerPhotoPreviews();
  };
    ...
    spreadsheetVisitHistories
      ...
      .sort(
        (
          ...
          // 同じ来店日なら、あとから登録された履歴を先にする
          const registeredAtComparison =
            (
              secondVisit.registeredAt ||
              ""
            ).localeCompare(
              firstVisit.registeredAt ||
              ""
            );

          if (
            registeredAtComparison !==
            0
          ) {
            return registeredAtComparison;
          }

          // 登録日時まで同じ場合は、来店IDが新しい履歴を先にする
          return (
            secondVisit.visitId ||
            ""
          ).localeCompare(
...
const renderCustomerDetailGallery = () => {
  ...
  const galleryPhotos =
    Array.isArray(
      activeCustomerDetailData.galleryPhotos
    )
      ? activeCustomerDetailData.galleryPhotos
      : [];
  ...
  const galleryItems =
    galleryPhotos
      .map((photo, index) => {
        return `
          <div
            class="customer-detail-gallery-item customer-detail-gallery-item--photo"
          >
            <img
              class="customer-detail-gallery-image"
              src="${photo.previewUrl}"
              alt="顧客写真 ${index + 1}枚目"
            >

            <button
              class="customer-detail-gallery-representative${
                photo.isRepresentative
                  ? " customer-detail-gallery-representative--active"
                  : ""
              }"
              type="button"
            ...
            <button
              class="customer-detail-gallery-remove"
              type="button"
              data-photo-id="${photo.photoId}"
              aria-label="顧客写真 ${index + 1}枚目を削除"
            >
              ×
            </button>
          </div>
...
const renderCustomerDetailScreen = () => {
  ...
  const hasRepresentativePhoto =
    Boolean(
      customerData
        .representativePhotoPreviewUrl
    );

  customerDetailMainPhotoImage.hidden =
    !hasRepresentativePhoto;

  customerDetailAvatar.hidden =
    hasRepresentativePhoto;

  if (hasRepresentativePhoto) {
    customerDetailMainPhotoImage.src =
      customerData
        .representativePhotoPreviewUrl;
  } else {
    customerDetailMainPhotoImage.removeAttribute(
      "src"
    );
  }
...
// GASから顧客写真を取得して詳細画面へ反映する
const loadCustomerDetailPhotos = (
  customerId
) => {
  if (
    !window.google?.script?.run ||
...
const renderCustomerDetail = (
  ...
  // 顧客検索カードに表示されている代表写真を取得する
  const representativePhotoPreviewUrl =
    resultCard.querySelector(
      ".customer-search-result-avatar-image"
    )?.src || "";
...
// 顧客詳細の保存中状態をまとめて切り替える
const setCustomerDetailEditSavingState = (
  isSaving
) => {
  isCustomerDetailEditSaving =
    isSaving;
  ...
  // ×ボタンはフォーム外なので個別にロックする
  customerDetailEditCloseButton.disabled =
    isSaving;

  customerDetailEditSaveButton.textContent =
    isSaving
      ? "保存中..."
      : "保存する";
};

// 顧客詳細編集で使用できる特徴タグ名を取得する
const getCustomerDetailEditFeatureNames =
  () => {
    ...
    const customerFeatureNames =
      activeCustomerDetailData
        ?.features ||
      [];

    // 標準タグと、その顧客だけが持っているタグをまとめる
    return Array.from(
      new Set([
        ...standardFeatureNames,
        ...customerFeatureNames,
      ])
    ).filter(Boolean);
  };
...
// 顧客詳細編集用の特徴タグを1個作る
const createCustomerDetailEditFeatureChip =
  (
    featureName,
    isSelected
  ) => {
    ...
    button.className =
      "feature-chip customer-detail-edit-feature-chip";
...
// 編集画面へ特徴タグを表示する
const renderCustomerDetailEditFeatures =
  () => {
    if (
      !activeCustomerDetailData
    ) {
    ...
    getCustomerDetailEditFeatureNames()
      .forEach(
        (featureName) => {
          const button =
            createCustomerDetailEditFeatureChip(
              featureName,
    ...
    customerDetailEditFeatureInput.value =
      "";
  };


// 顧客詳細編集から新しい特徴タグを追加する
const addCustomerDetailEditFeature =
  () => {
    const featureName =
      customerDetailEditFeatureInput
        .value
        .trim();
    ...
    const currentButtons =
      Array.from(
        customerDetailEditFeatureList
          .querySelectorAll(
            ".customer-detail-edit-feature-chip"
          )
      );
    ...
    customerDetailEditFeatureInput.disabled =
      true;

    customerDetailEditFeatureAddButton.disabled =
      true;
    ...
    google.script.run
      .withSuccessHandler(
        (featureTag) => {
          ...
          customerDetailEditFeatureInput.focus();
        }
      )
      .withFailureHandler(
        (error) => {
...
customerDetailEditFeatureAddButton
  .addEventListener(
    "click",
    addCustomerDetailEditFeature
  );
...
const openCustomerDetailEditDialog = () => {
  ...
  renderCustomerDetailEditFeatures();
...
customerDetailEditForm.addEventListener(
  ...
  (event) => {
    ...
    // 保存開始と同時に編集画面全体をロックする
    setCustomerDetailEditSavingState(
      true
    );
...
// GASから返った写真一覧を現在の顧客へ反映する
const applySavedCustomerDetailPhotos = (
  photos
) => {
  if (!activeCustomerDetailData) {
    return;
...
customerDetailGalleryList.addEventListener(
  ...
  (event) => {
    const representativeButton =
      event.target.closest(
        ".customer-detail-gallery-representative"
      );
    ...
    if (representativeButton) {
      ...
      google.script.run
        .withSuccessHandler(
          (photos) => {
            applySavedCustomerDetailPhotos(
              photos
            );
        ...
        .setRepresentativeCustomerPhoto(
          activeCustomerDetailData.id,
          representativeButton.dataset
            .photoId
        );
    ...
    const removeButton =
      event.target.closest(
        ".customer-detail-gallery-remove"
      );
    ...
    customerDetailPhotoInput.click();
  }
);


// 顧客詳細から写真をDriveへ追加する
const addCustomerDetailPhotoFiles =
  async (files) => {
    if (
      !activeCustomerDetailData
    ) {
    ...
    const newFiles =
      imageFiles.filter(
        (file) => {
          return !activeCustomerDetailData
            .galleryPhotos
            .some((photo) => {
              return (
                photo.fileName ===
    ...
    try {
      const photoDataList =
        await createCustomerPhotoDataListForGas(
          newFiles
        );
...
// ファイル選択画面から写真を選んだとき
customerDetailPhotoInput.addEventListener(
  "change",
  () => {
    addCustomerDetailPhotoFiles(
      customerDetailPhotoInput.files
    );

    // 同じ写真を再選択できるよう入力欄を空にする
    customerDetailPhotoInput.value = "";
  }
);
...
customerDetailGalleryList.addEventListener(
  ...
  (event) => {
    ...
    customerDetailGalleryList.classList.add(
      "customer-detail-gallery-list--dragging"
    );
  }
);
...
customerDetailGalleryList.addEventListener(
  ...
  (event) => {
    ...
    if (
      event.relatedTarget &&
      customerDetailGalleryList.contains(
        event.relatedTarget
      )
    ) {
      return;
    ...
    customerDetailGalleryList.classList.remove(
      "customer-detail-gallery-list--dragging"
    );
  }
);
...
// サマリーの集計期間に使用する月数
const summaryPeriodMonthCounts = {
  month: 1,
  "three-months": 3,
  "six-months": 6,
  year: 12,
...
// 選択月をDate型へ変換する
const createSummarySelectedMonthDate =
  () => {
    const [
      yearText,
      monthText,
...
// 比較用の割合を表示形式へ整える
const formatSummaryComparisonRate = (
  currentValue,
  previousValue
) => {
  if (previousValue === 0) {
...
// 数値の差を符号つきで整える
const formatSummaryNumberDifference = (
  difference,
  suffix
) => {
  return (
...
// 金額の差を符号つきで整える
const formatSummaryCurrencyDifference = (
  difference
) => {
  ...
  return (
    `${
      ...
      absoluteDifference.toLocaleString(
        "ja-JP"
      )
    }`
  );
...
// 選択月・選択期間のKPIを実データから計算する
const getRealSummaryKpiData =
  () => {
    const selectedMonthDate =
      createSummarySelectedMonthDate();

    const periodMonthCount =
      summaryPeriodMonthCounts[
        activeSummaryPeriod
      ] ||
      1;
    ...
    const currentNewCustomerCount =
      currentCustomerCards.length;

    const previousNewCustomerCount =
      previousCustomerCards.length;

    const currentAverageSpend =
      currentVisitCount > 0
        ? Math.round(
            currentSales /
            currentVisitCount
    ...
    return {
      ...
      salesRate:
        formatSummaryComparisonRate(
          currentSales,
          previousSales
        ),

      salesDifference:
        formatSummaryCurrencyDifference(
          currentSales -
          previousSales
        ),
...
// サマリーで選択中の集計期間を取得する
const getSummaryCurrentPeriodRange =
  () => {
    const selectedMonthDate =
      createSummarySelectedMonthDate();
...
// 来店履歴を古い順へ並べる
const getSummaryVisitHistoriesOldestFirst =
  () => {
    return [
      ...spreadsheetVisitHistories,
    ].sort(
...
// 新規・常連の来店回数を実データから計算する
const getRealSummaryRatioData =
  () => {
    const {
      startDate,
      endDate,
    ...
    const firstVisitByCustomerId =
      new Map();

    getSummaryVisitHistoriesOldestFirst()
      .forEach((visitHistory) => {
        const customerId =
          visitHistory.customerId ||
          "";
        ...
        if (
          customerId &&
          !firstVisitByCustomerId.has(
            customerId
          )
        ) {
          firstVisitByCustomerId.set(
            customerId,
            visitHistory
          );
        }
          ...
          >
            ${
              photoPreviewUrl
                ? `
                  <img
                    class="customer-search-result-avatar-image"
                    src="${escapeCustomerSearchHtml(
                      photoPreviewUrl
                    )}"
                    alt=""
                  >
...
// Sheetsの特徴タグからボタンを1個作る
const createFeatureTagMasterButton = (
  featureTag,
  isSelected = false
) => {
  const button =
...
// Sheetsから取得した特徴タグ一覧を表示する
const renderFeatureTagMaster = (
  featureTags
) => {
  // 再描画前に、現在選択しているタグIDを覚えておく
  const selectedFeatureTagIds =
    new Set(
      Array.from(
        featureList.querySelectorAll(
          ".feature-chip--selected"
...
// 「削除する」を押した場合は
// Sheetsのタグマスタを無効化する
featureDeleteDialogConfirm.addEventListener(
  "click",
  () => {
    const tagIds =
      pendingDeleteFeatureButtons
    ...
    featureDeleteDialogConfirm.disabled =
      true;

    featureDeleteDialogCancel.disabled =
      true;

    featureDeleteDialogConfirm.textContent =
      "削除中...";
...
const addCustomFeature = () => {
  ...
  customFeatureInput.disabled =
    true;

  featureAddButton.disabled =
    true;
...
// 新規顧客の代表写真として使用する位置
let customerPhotoRepresentativeIndex = 0;
...
// GASへ送る写真の最大辺
const customerPhotoMaxSize = 1600;

// JPEG / WebPで使用する圧縮品質
const customerPhotoCompressionQuality = 0.82;


// 写真ファイルを縮小してGAS送信用Base64へ変換する
const createCustomerPhotoDataForGas = (
  file
) => {
  return new Promise(
    (resolve, reject) => {
      // 元画像をブラウザ内だけで読み込むURLを作る
      const imageUrl =
        URL.createObjectURL(
          file
        );
        ...
        () => {
          try {
            ...
            const result =
              canvas.toDataURL(
                outputMimeType,
                customerPhotoCompressionQuality
              );
...
// 複数写真をまとめてGAS送信用へ変換する
const createCustomerPhotoDataListForGas =
  async (files) => {
    return Promise.all(
      Array.from(files).map(
        createCustomerPhotoDataForGas
      )
    );
  };
...
const renderCustomerPhotoPreviews = () => {
  ...
  if (
    customerPhotoFiles.length === 0
  ) {
    customerPhotoRepresentativeIndex = 0;

    photoPreviewPlaceholder.hidden =
      false;
  ...
  customerPhotoFiles.forEach(
    (file, index) => {
      ...
      representativeButton.type =
        "button";

      const isRepresentative =
        index ===
        customerPhotoRepresentativeIndex;

      representativeButton.className =
        "photo-preview-item-representative";

      if (isRepresentative) {
        representativeButton.classList.add(
          "photo-preview-item-representative--active"
        );
      }

      representativeButton.textContent =
        isRepresentative
          ? "★ 代表写真"
          : "☆ 代表に設定";

      representativeButton.addEventListener(
        "click",
        () => {
          customerPhotoRepresentativeIndex =
            index;
...
regularCustomerConfirmUpdate.addEventListener(
  ...
  () => {
    ...
    // 更新中は「戻って修正」も押せないようにする
    regularCustomerConfirmBack.disabled =
      true;
...
newCustomerConfirmRegister.addEventListener(
  "click",
  async () => {
    newCustomerConfirmRegister.disabled =
      true;

    // 登録中は「戻って修正」も操作できないようにする
    newCustomerConfirmBack.disabled =
      true;
      ...
      google.script.run
        .withSuccessHandler(
          (result) => {
            ...
            newCustomerConfirmMain.hidden =
              true;

            newCustomerRegisterSuccess.hidden =
              false;
